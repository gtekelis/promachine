...
@section('content')
    <div class="p-6 max-w-lg mx-auto bg-white flex items-center rounded-lg">
        <form action="{{ route('eanList') }}" method="post" class="flex-auto mb-4">
            @csrf
            <div class="mb-4">
                <label for="body" class="sr-only">Body</label>
                <textarea name="ean" id="ean" cols="30" rows="4"
                          class="bg-gray-100 border-2 w-full p-4 rounded-lg @error('ean') border-red-500 @enderror"
                          placeholder="Ένα ή περισσότερα EAN"></textarea>
                @error('ean')
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        @foreach($ean_list as $ean)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <form action="{{ route('eanList.delete', $ean) }}" method="post">
                                        @csrf
                                        @method('DELETE')
